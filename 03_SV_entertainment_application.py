...
class EntertainmentExpence:
    ...
    def getCompanyPublicEnemyCd(self):     # --- ADD NEW 2022.01.07 --- おきゃくたまー等と言いながら、反社チェック。ええで、そういう二面性。大事にしいや
        wk2PublicEnemyCd = ""
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        while sheet.cell(row=i, column=1).value is not None:
            ee = EntertainmentExpence(sheet.cell(row=i, column=9).value,    # CREATOR
                                ...
                                sheet.cell(row=i, column=119).value,         # 先方企業名#1  <--- 東証一部上場企業もチェック対象やで。司の親分、高山の若頭（カシラ）もびっくりやで。
                                sheet.cell(row=i, column=120).value,         # 先方企業パブリックエネミーコード#1
                                sheet.cell(row=i, column=121).value,         # 先方企業名#2
                                sheet.cell(row=i, column=122).value,         # 先方企業パブリックエネミーコード#2
                                sheet.cell(row=i, column=123).value,         # 先方企業名#3
                                sheet.cell(row=i, column=124).value,         # 先方企業パブリックエネミーコード#3
                                sheet.cell(row=i, column=125).value,         # 先方企業名#4
                                sheet.cell(row=i, column=126).value,         # 先方企業パブリックエネミーコード#4
                                sheet.cell(row=i, column=127).value,         # 店舗名#4
                                sheet.cell(row=i, column=126).value,          # 店舗パブリックエネミーコード#4
            ...
            logger.info(  # --- ADD NEW 個別 20212.01.04
                "先方役職: " + str(ee.getPosition())                                          # --- あてつけがましさ炸裂やね（おんどれ何様やねんワレ）
            )
            ...
            logger.info(  # --- ADD NEW 個別 20212.01.04
                # "先方反社コード: " + str(ee.getPublicEnemyCd())                               # --- 六代目山口組は6、三代目弘道会は3、らしいで
            ...
            driver.save_screenshot(f'C:/Users/digiworker_biz_02/PycharmProjects/pythonProject/ScreenShot/SV/{i}_申請ボタンクリック.png')
