# 【経費】交通費精算書（高速代、ガス代、駐車場代等） <--- 電車賃からフォークしたプログラム
import logging
...
logger = logging.getLogger("1.1")    # 宣言場所を移動。 --- 2021.11.25 (Nakabayashi)
...
class SettlementInfo:       # --- 2022.01.08 ADD NEW 決裁情報  --- ※電車賃システムから移設
    def __init__(self,
                 wkIid,
                 wkDocno,
                 wkFormname,
...
#   logger = logging.getLogger("01")    # 場所を移動。 --- 2021.11.25 (Nakabayashi)
    ...
    file_handler = logging.FileHandler("../Logs/01.1_transport_application_Highway_Parking_etc.log")    # --- UPDATE 2022.01.04
...
def entry_procedure():
    ...
    # 交通費精算ファイルであるT3.xlsxへアクセス - 2021.11.16
    book = openpyxl.load_workbook("C:\XLSX\XM_SLIP_DETAIL\T3.xlsx")     # Change 2021.12.27
    sheet = book['T3']
    logger.info(  # --- ADD NEW 共通部品 20212.01.04
        "T3.xlsxファイルオープン完了"
    )
    # T3エクセルブックを2行目（1行目は見出し行）から読込み→OpenPyXlを使用する。Pandasは排除。
    i = 2
    while sheet.cell(row=i, column=1).value is not None:
        ...
        entry_items(sheet.cell(row=i, column=45).value,     # 1.行き先[AS]
                    ...
                    i,                                      # T3シートの行数（見出し行含む） --- ADD NEW 2022.01.05
...
def select_specification_kind():
    ...
    transport_type = "その他費用-立替(実費精算)"      # --- 2021.12.28 UPDATE --- ガス代、高速通行料等Ver.への焼直し
...
def entry_items(destination,
                ...
                pdf_voucher8,
                pdf_voucher9,
                remark,         # --- 2021.12.28 ADD NEW
                gyou,           # --- 2022.01.05 ADD NEW
    ...
    # 利用区分エントリー処理
    logger.info(  # --- ADD NEW 個別 20212.01.04
        "変換前旧MF利用区分コード: " + usageClassificationCd
    )
    ...
    # if detailedRoute is not None:                     # --- INVALID 2022.01.08 --- Start
    ...
    driver.save_screenshot(f'C:/Users/digiworker_biz_02/PycharmProjects/pythonProject/ScreenShot/T3/{gyou}_確定ボタンクリック.png')
...
def click_appeal_button(gyou):      # UPDATE 2021.11.22
    ...
    driver.save_screenshot(f'C:/Users/digiworker_biz_02/PycharmProjects/pythonProject/ScreenShot/T3/{gyou}_申請ボタンクリック.png')
